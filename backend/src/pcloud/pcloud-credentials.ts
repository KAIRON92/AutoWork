...
function getKey(): Buffer {
  ...
  if (key.length !== 32) {
    throw new Error('PCLOUD_CREDENTIAL_ENCRYPTION_KEY must be a base64-encoded 32-byte key');
  }
  return key;
}
...
export function decryptPCloudCredential(value: string): string {
  if (!value.startsWith(`${VERSION}.`)) {
    throw new Error('Stored pCloud credential is not encrypted with the current credential format');
  }

  const [, ivB64, tagB64, dataB64] = value.split('.');
  if (!ivB64 || !tagB64 || !dataB64) {
    throw new Error('Invalid encrypted pCloud credential format');
  }
