...
export class CampaignsService {
  ...
  async findAll(organizationId: string) {
    return this.prisma.campaign.findMany({
      ...
      include: {
        ...
        emailAccount: { select: { id: true, provider: true, accountEmail: true, displayName: true, status: true } },
  ...
  async create(organizationId: string, dto: CreateCampaignDto) {
    const [account, file, template, emailAccount] = await Promise.all([
      ...
      dto.emailAccountId ? this.prisma.emailAccount.findFirst({ where: { id: dto.emailAccountId, organizationId } }) : Promise.resolve(null),
    ]);
    ...
    if (dto.emailAccountId) {
      if (!emailAccount) throw new BadRequestException(`Invalid sender account ${dto.emailAccountId}`);
      if (emailAccount.status !== 'VERIFIED') throw new BadRequestException('Selected sender account is not verified');
    }

    let contactIds: { id: string; email: string }[] = [];
    if (dto.contactListId) {
      const contactList = await this.prisma.contactList.findFirst({ where: { id: dto.contactListId, organizationId } });
    ...
    const campaign = await this.prisma.campaign.create({
      data: {
        ...
        emailAccountId: dto.emailAccountId || null,
  ...
  async launch(id: string, organizationId: string) {
    ...
    const campaign = await this.prisma.campaign.findFirst({
      where: { id, organizationId },
      include: { pcloudAccount: true, pcloudFile: true, emailAccount: true, template: true, recipients: true },
    });
    if (!campaign) throw new NotFoundException(`Campaign ${id} not found`);
    if (!['DRAFT', 'PAUSED'].includes(campaign.status)) throw new BadRequestException(`Campaign cannot be launched from ${campaign.status} state`);
    ...
    if (!campaign.emailAccount || campaign.emailAccount.status !== 'VERIFIED') {
      throw new BadRequestException('A verified sender account is required before launching a campaign');
    }

    const config = campaign.config ? JSON.parse(campaign.config) : { shareType: 'uploadtransfer' };
    if (config.shareType !== 'uploadtransfer' && config.shareType !== 'sharefolder') throw new BadRequestException(`Unsupported pCloud operation: ${config.shareType}`);
    ...
    return { message: 'Campaign queued for pCloud processing', campaignId: campaign.id, status: 'QUEUED', totalRecipients: campaign.recipients.length };
  }
